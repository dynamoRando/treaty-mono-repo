use crate::{
    ...
    treaty_proto::{RowInfo, TreatyError, UpdateDataRequest, UpdateDataResult},
};
    ...
    match result {
        Ok(opt_known_host) => {
            if let Some(known_host) = opt_known_host {
                let result = db.get_updates_from_host_behavior(db_name, table_name).await;
                match result {
                    Ok(update_behavior) => match update_behavior {
                            ...
                            match result {
                                Ok(data_result) => {
                                    if data_result.is_successful {
                                        ...
                                        message = String::from(
                                            "The update statement has been logged for review",
                                        );
                                        is_successful = true;
                                    }
                                }
