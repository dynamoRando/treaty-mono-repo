...
impl<'input> ParseTreeListener<'input, SQLiteParserContextType> for TreatySqliteListener {
...
impl<'input> SQLiteListener<'input> for TreatySqliteListener {
    ...
    fn enter_select_core(&mut self, _ctx: &sqliteparser::Select_coreContext<'input>) {
        self.statement_type.data = DmlType::Select;
        // trace!("{:?}", self.statement_type);
        // trace!("treatySQLITELISTNER ENTERED SELECT CORE STATEMENT");
    }
